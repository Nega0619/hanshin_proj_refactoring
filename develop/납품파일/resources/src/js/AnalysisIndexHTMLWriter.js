...
function writeHTMLGraphs(disease, indexes) {
    for (let i = 0; i < 6; i++) {
        ...
        document.write(
            `<div class="indexName">- ${indexes[i]["name"].replace(
                /\s/g,
                ""
            )}</div>`
        ); //정규표현식 : 공백제거
        ...
        document.write("</div>");
        document.write(
            "<div class='indexValue'>" + indexes[i]["value"] + "</div>"
        );
        document.write("</div>");
    }
}
