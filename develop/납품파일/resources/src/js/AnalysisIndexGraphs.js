...
class GraphDrawer {
    ...
    drawGraphs() {
        ...
        for (let i = 0; i < 6; i++) {
            const min = this.indexes[i]["min"];
            const max = this.indexes[i]["max"];
            const value = this.indexes[i]["value"];

            if (this.indexes[i]["name"] == "요단백") {
                const 요단백 = new 요단백Graph(this.disease);
                요단백.drawGraph(i, min, max, value);
            } else if (this.indexes[i]["name"] == "비만도") {
                const 비만도 = new 비만도Graph(this.disease);
                비만도.drawGraph(i, min, max, value);
            } else if (this.indexes[i]["name"] == "고밀도지단백콜레스테롤") {
